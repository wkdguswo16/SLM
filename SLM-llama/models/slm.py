...
class ScalableLM(PreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        if config.retriever_state_dict is not None:
            self.retriever.keys = nn.parameter.Parameter(torch.load(
                config.retriever_state_dict)['keys'], requires_grad=False)
    ...
    def restore_other_task_weight_offset(self, task):
        ...
        self.retriever.weight_offset[:,
                                     :l_idx, ...] = self.src_weight_offset[:, :l_idx, ...]
        self.retriever.weight_offset[:,
                                     r_idx:, ...] = self.src_weight_offset[:, r_idx:, ...]
        self.retriever.weight_offset.requires_grad = True

    # Filter out the tasks not in the test list

    def get_task_list_mask(self):
        mask = torch.zeros(self.config.pool_size,
                           dtype=torch.int, device=self.retriever.keys.device)
        ...
        **kwargs,
            ...
            for names in shared_ptrs.values():
                ...
                if self._keys_to_ignore_on_load_missing is not None:
                    ...
                    for name in sorted(names):
                        matches_pattern = any(
                            re.search(pat, name) for pat in self._keys_to_ignore_on_load_missing)
        ...
        if index is None:
            path_to_weights = os.path.join(
                save_directory, _add_variant(WEIGHTS_NAME, variant))
