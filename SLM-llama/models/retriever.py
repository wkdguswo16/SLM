...
class Retriever(nn.Module):
    def __init__(self, config: LlamaCLConfig) -> None:
        super().__init__()
        self.config = config
        assert config.similarity_type in [
            'cosine', 'softmax'], "The similarity calculation should be ['cosine', softmax]"
        ...
        self.model_hidden_size = self.bert.get_input_embeddings(
        ).weight.shape[-1]
        assert self.model_hidden_size % self.groups == 0, "The vectordb hidden size: {} can not be divided envenly by the groups:{}".format(
            self.model_hidden_size, self.groups)
        self.key_hidden_size = self.model_hidden_size // self.groups

        # weight offset
        low_rank_a = torch.zeros(
            self.pool_size, self.hidden_size * self.config.low_rank * self.num_hidden_layers)
        low_rank_b = nn.init.normal_(torch.empty(
            self.pool_size,  self.hidden_size * self.config.low_rank * self.num_hidden_layers))
        self.weight_offset = nn.parameter.Parameter(torch.stack(
            [low_rank_a, low_rank_b], dim=-2))  # [pool_size, 2, channels*l*r]

        if self.pool_size > self.key_hidden_size:
            warnings.warn(
                "The pool size is larger than the key_hidden_size, may cause the generate unstable keys")
        keys = [generate_orthogonal_matrix(
            self.pool_size, self.key_hidden_size) for _ in range(self.groups)]
        # [1, groups, pool_size, key_hidden_size]
    ...
    def forward(
        ...
        if self.similarity_type == 'cosine':
            ...
            # [bsz, groups, pool_size]
            sim = F.cosine_similarity(queries, keys, dim=-1)
        else:
            queries = queries.unsqueeze(2).repeat(1, 1, self.pool_size, 1)
            # [bsz, groups, pool_size]
            sim = F.cosine_similarity(queries, keys, dim=-1) / 0.1
        ...
        if not use_distance_weight:
            ...
            bin_count = torch.bincount(
                idx_vote, minlength=self.pool_size*self.groups).view(self.groups, self.pool_size)
            idx_vote = torch.topk(bin_count, k=self.weight_topk)[
                1]  # [groups, topk]
        else:
        ...
        if not use_distance_weight:
            ...
            weight_offset = (
                dis_weight[:, None, None, None] * weight_offset).sum(0)
        ...
        if self.pool_train_keys:
            ...
            if self.last_keys is not None:
                outputs['centrifugal_loss'] = torch.einsum("b g x c, b g y c -> b g x y", F.normalize(
                    self.keys, dim=-1), F.normalize(self.last_keys, dim=-1)).mean()
    ...
    def encode(self, inps):
        with torch.no_grad():
            ...
            if isinstance(inps, Sequence):
                inps = self.tokenizer(inps, padding=True,
                                      truncation=True, return_tensors='pt')
            assert isinstance(
                inps, BatchEncoding), "The inputs of the encoder should be BatchEncoding."
    ...
    def set_keys(self, values):
        assert values.shape == torch.Size([1, self.groups, self.pool_size, self.key_hidden_size]), "The shape of values: {} don't equal to {}".format(
            values.shape, [1, self.groups, self.pool_size, self.key_hidden_size])
        self.keys = nn.parameter.Parameter(values)
        if not self.pool_train_keys:
            self.freeze_keys()

    def set_last_keys(self, values):
        assert values.size(-1) == self.key_hidden_size and values.size(
            1) == self.groups, "The last keys shape: {} doesn't fit the retriever shape: {}".format(values.shape, self.keys.shape)
